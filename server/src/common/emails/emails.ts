...
class EmailSender {
    ...
    sendEmailConfirmationEmail = async (receiver: string, username: string, frontendBaseUrl: string, frontendVerificationUrl: string) => {
        const html = this.templates.emailConfirmation({username, frontendBaseUrl, frontendVerificationUrl})
        const mailOptions = {
            from: env.SMTP_SENDER, to: receiver, subject: files.emailConfirmation.subject, html: html
        }
