...
function generate_route_object(route, url, manifest) {
	...
	const nodes = [...errors, ...layouts.map((l) => l?.[1]), leaf[1]]
		...
		.map(
			(n) => `'${n}': () => ${create_client_import(manifest._.client.nodes?.[n], url)}`
		)
		.join(',\n		');
	return [
		`{
			...
			fetch: async (info2, init3) => {
				...
				if (set_cookie) {
					for (const str of splitCookiesString_1(set_cookie)) {
						...
						const path =
							options3.path ?? (url.pathname.split('/').slice(0, -1).join('/') || '/');
...
async function respond(request, options2, manifest, state) {
	...
	try {
		...
		resolved_path =
			(await options2.hooks.reroute({ url: new URL(url), fetch: event.fetch })) ??
			url.pathname;
	} catch {
