...
import {
	d as darkClassNames,
	a as disableTransitions,
	l as lightClassNames,
	m as modeStorageKey,
	p as parse,
	b as themeColors,
	t as themeStorageKey
} from '../../chunks/states.svelte.js';
...
function Footer($$payload, $$props) {
	...
	const linkCategories = [
		...
		/** TODO: Update once this href is known. */
		{
			...
			links: [
				...
				{
					label: 'Email',
					href: 'mailto:contact@verdagraph.org'
				},
				{
	...
	const each_array = ensure_array_like(linkCategories);
	$$payload.out += `<footer class="bg-neutral-2 border-neutral-6 mt-8 flex w-full justify-around border-t py-12"><!--[-->`;
	for (
		let $$index_1 = 0, $$length = each_array.length;
		$$index_1 < $$length;
		$$index_1++
	) {
		let linkCategory = each_array[$$index_1];
		const each_array_1 = ensure_array_like(linkCategory.links);
		$$payload.out += `<ul class="flex flex-col"><span class="text-neutral-10 mb-4 text-sm">${escape_html(linkCategory.label)}</span> <!--[-->`;
		for (
...
function Nav($$payload, $$props) {
	...
	$$payload.out += `<header class="border-neutral-6 sticky left-0 top-0 w-full rounded-none border-b drop-shadow-md"><nav class="bg-neutral-2 flex items-center justify-between px-8 py-2"><div><ul class="flex items-center gap-6 p-2 text-lg"><li><a href="/"><span class="font-semibold">Verdagraph</span></a></li></ul></div> <ul class="hidden gap-4 md:flex md:gap-8 lg:gap-12"><!--[-->`;
	...
	$$payload.out += `<!--]--></ul> <div class="flex lg:hidden">`;
	Root($$payload, {
		children: ($$payload2) => {
			Trigger($$payload2, {
				children: ($$payload3) => {
			...
			Popover_content($$payload2, {
				...
				children: ($$payload3) => {
					...
					$$payload3.out += `<!--]--> `;
					Separator($$payload3, { class: 'bg-neutral-6 my-2 w-full opacity-50' });
					$$payload3.out += `<!----> <li>`;
					menuLink($$payload3, env.APP_URL, 'Start the Application');
					$$payload3.out += `<!----></li></ul>`;
				},
				$$slots: { default: true }
			});
...
function _layout($$payload, $$props) {
	...
	$$payload.out += `<!----> <div class="h-screen w-screen"><!---->`;
	Provider($$payload, {
		delayDuration: 500,
		children: ($$payload2) => {
