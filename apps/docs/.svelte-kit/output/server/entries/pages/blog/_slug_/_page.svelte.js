...
function _page($$payload, $$props) {
  ...
  MainContentContainer($$payload, {
    children: ($$payload2) => {
      $$payload2.out += `<article><hgroup><h1>${escape_html(data.meta.title)}</h1> <p class="text-neutral-10">Published ${escape_html(df.format(new Date(data.meta.updatedDate)))}</p></hgroup> <div><!---->`;
