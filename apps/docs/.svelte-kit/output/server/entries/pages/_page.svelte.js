...
import { I as Icon$1, o as onDestroy } from '../../chunks/Icon.js';
import { B as Button, c as cn, e as env } from '../../chunks/env.js';
import {
	B as assign_payload,
	n as attr,
	I as attr_class,
...
function createCanvasGridManager(container, transform) {
	...
	const pixelsPerBackgroundGridline =
...
const germToTransplantSchema = z
	.number()
	.min(0, 'May not be negative.')
	.describe(
...
const firstToLastHarvestSchema = z
	.number()
	.min(0, 'May not be negative.')
	.describe(
...
const AnnualLifecycleUpdateCommandSchema = z
	.object({
...
const lastFrostWindowOpenSchema = z
	.number()
	.describe(
...
const lastFrostWindowCloseSchema = z
	.number()
	.describe(
...
const firstFrostWindowOpenSchema = z
	.number()
	.describe(
...
const firstFrostWindowCloseSchema = z
	.number()
	.describe(
...
const FrostDatePlantingWindowsUpdateCommandSchema = z
	.object({
...
const OriginUpdateCommandSchema = z
	.object({
...
const GardenMembershipStatusEnumOptions = ['CREATED', 'PENDING', 'ACCEPTED'];
...
const emailSchema = z$1
	.string()
	.email('Must be a valid email address.')
	.describe('Must be a valid email address.');
const passwordSchema = z$1
	.string()
...
const usernameInvitesListSchema = z$1
...
const GeometryTypeEnumOptions = ['RECTANGLE', 'POLYGON', 'ELLIPSE', 'LINES'];
const workspaceSchema = Schema.Collections({
	...
	geometries: {
		schema: Schema.Schema({
			...
			linesCoordinateIds: Schema.Set(Schema.String(), {
				default: Schema.Default.Set.empty()
			}),
...
const AnnualTemperatureUpdateCommandSchema = z
	.object({
...
const FrostDatesUpdateCommandSchema = z
	.object({
...
const HarvestQualityEnumOptions = ['COMPOST', 'LOW', 'MEDIUM', 'HIGH', 'PERFECT'];
Schema.Collections({
	...environmentSchema,
	/** Harvest schema. */
	harvests: {
	...
	lifespans: {
		...
		relationships: {
			...
			locationHistory: Schema.RelationById('locationHistories', '$locationHistoryId'),
			harvests: Schema.RelationMany('harvests', {
...
const geometryRotationSchema = z$1
	.number()
...
const geometryLinesCoordinatesSchema = z$1
...
const geometryLinesClosedSchema = z$1
	.boolean()
...
const CultivarCollectionVisibilityEnumOptions = ['HIDDEN', 'UNLISTED', 'PUBLIC'];
Schema.Collections({
	...environmentSchema,
	/** Collection schema. */
	cultivarCollections: {
...
function EditableGeometryResizePoints($$payload, $$props) {
	push();
	let { canvasId, geometry, geometryGroup } = $$props;
	getContext(canvasId);
