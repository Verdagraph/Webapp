...
function getStorage(provider, prefix) {
	...
	return (
		providerStorage[prefix] || (providerStorage[prefix] = newStorage(provider, prefix))
	);
}
...
function replaceIDs(body, prefix = randomPrefix) {
	...
	ids.forEach((id) => {
		const newID =
			typeof prefix === 'function' ? prefix(id) : prefix + (counter++).toString();
...
function listToIcons(list, validate = true, simpleNames2 = false) {
	...
	list.forEach((item) => {
		const icon =
			typeof item === 'string' ? stringToIcon(item, validate, simpleNames2) : item;
		if (icon) {
			result.push(icon);
		}
	});
...
function render(icon, props) {
	...
	const useMask =
		mode === 'mask' || (mode === 'bg' ? false : body.indexOf('currentColor') !== -1);
	...
	});
	{
		...
		if (data && iconData.classes) {
			data.attributes['class'] =
				(typeof $$sanitized_props['class'] === 'string'
...
export { Icon as I, html as h, onDestroy as o, tick as t };
