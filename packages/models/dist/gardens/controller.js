import { AppError, GardenCreateCommandSchema, GardenMembershipAcceptCommandSchema, GardenMembershipCreateCommandSchema, GardenMembershipDeleteCommandSchema, GardenMembershipRevokeCommandSchema, GardenMembershipRoleChangeCommandSchema, isProfileMember, isUserAuthorized } from '@vdg-webapp/models';
import { getClientOrError } from '../users/controller.js';
 ...
 */
export async function requireRole(gardenId, action, ctx) {
    /** Retrieve client. */
    const client = await getClientOrError(ctx);
    /** Retrieve garden. */
    const garden = await ctx.triplit.fetchOne(gardenQuery.Vars({ id: gardenId }));
    if (garden == null) {
 ...
 */
async function getNewMembershipIdsFromUsernames(usernames, triplit, garden) {
    if (!usernames) {
        return new Set();
    }
...
export const gardenMembershipAccept = {
    ...
    mutation: async function (data) {
        ...
        const membership = await triplit.fetchOne(membershipQuery.Vars({ gardenId: data.gardenId, userId: client.profile.id }));
        if (!membership) {
