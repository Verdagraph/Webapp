 ...
 */
export function historySelect(items, date) {
    if (items.length === 0) {
        return null;
    }
 ...
 * @returns The item at the given date.
 */
export function historySelectDay(items, date) {
 ...
 */
export function getGeometryHeight(geometry) {
    switch (geometry.type) {
        ...
        case 'LINES':
            return (Math.max(...geometry.linesCoordinates.map((coordinate) => coordinate.y)) *
