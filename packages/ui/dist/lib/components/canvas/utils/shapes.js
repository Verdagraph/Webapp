 ...
 */
function getClosedOrUnclosedShape(canvas, geometry, forceLinesClosed = false, config, position) {
        ...
        case 'LINES':
            return new Konva.Line({
                points: geometry.linesCoordinates.reduce((output, coordinate) => {
                    output.push(canvas.transform.canvasXPos(coordinate.x), canvas.transform.canvasYPos(coordinate.y));
                    return output;
                }, []),
...
export function updateShape(canvas, newGeometry, shape) {
    if (shape instanceof Konva.Rect) {
        ...
        if (newGeometry.linesCoordinates) {
            shape.points(newGeometry.linesCoordinates.reduce((output, coordinate) => {
                output.push(canvas.transform.canvasXPos(coordinate.x), canvas.transform.canvasYPos(coordinate.y));
                return output;
            }, []));
        }
