...
import { calculateDeltaDays, calendarDateToUtc } from '../../../components/timeline/utils';
...
const defaultSliderDisplayOffset = {
    weeks: 2
};
...
export function createTimelineSelection() {
    ...
    let focus = $state(today(getLocalTimeZone()));
    /** Day which marks the start of the timeline selection. */
    let beginSelection = $state(focus.subtract(defaultLowerSelectionOffset));
    /** Day which marks the end of the timeline selection. */
    let endSelection = $state(focus.add(defaultUpperSelectionOffset));
    /** Derived selection. */
    const focusUtc = $derived(calendarDateToUtc(focus));
    const beginSelectionUtc = $derived(calendarDateToUtc(beginSelection));
    const endSelectionUtc = $derived(calendarDateToUtc(endSelection));
    /**
     * Slider properties.
     */
    /** Day which marks the start of the timeline displayed on the slider graphic. */
    let beginSlider = $state(beginSelection.subtract(defaultSliderDisplayOffset));
    /** Day which marks the end of the timeline displayed on the slider graphic. */
    let endSlider = $state(endSelection.add(defaultSliderDisplayOffset));
    ...
    const translateSliderForward = $derived(maxSliderValue - sliderValue[2] < translateRangeThreshold);
    const translateSliderBackward = $derived(sliderValue[0] - minSliderValue < translateRangeThreshold);
    let sliderExpandIntervalId = null;
    $effect(() => {
     ...
     */
    function sliderValueToDateValue(sliderValue) {
