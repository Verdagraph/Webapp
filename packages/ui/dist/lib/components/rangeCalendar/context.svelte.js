...
import { CalendarItem } from '../../../components/rangeCalendar/types';
import {} from '../../../components/timeline';
...
export function createCalendarContainerContext(timeline, config) {
    ...
    let sectionHeight = $derived(DEFAULT_SECTION_HEIGHT * (config.value.sectionHeight / 100));
 ...
 */
export function createCalendarPaneContext(spec, container) {
    let height = $state(0);
    /** The Melt-UI builder. */
    const tree = new Tree({
        items: spec.items,
