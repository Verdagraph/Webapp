...
const userFields = {
	username: z
		...
		.regex(/^[a-zA-Z0-9_]*$/, 'Must contain only alphanumeric characters and underscores.')
		.describe('Must be between 3 and 50 characters long and contain only alphanumeric characters and underscores. Must be unique.'),
	email: z
		.string()
	...
	password: z
		...
		.regex(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*\W).*$/, 'Must contain at least one lowercase letter, one uppercase letter, one digit, and one special character.')
		.describe('Must be between 6 and 255 characters long and contain at least one lowercase letter, one uppercase letter, one digit, and one special character.')
};
 ...
 */
export const UserCreateCommand = z.object({
    email: userFields.email,
 ...
 */
export const UserUpdateCommand = z.object({
 ...
 */
export const UserConfirmPasswordResetCommand = z.object({
